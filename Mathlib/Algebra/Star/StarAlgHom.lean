...
@[simp]
protected theorem coe_coe {F : Type*} [NDFunLike F A B] [NonUnitalStarAlgHomClass F R A B] (f : F) :
    ⇑(f : A →⋆ₙₐ[R] B) = f := rfl
...
@[simp]
protected theorem coe_coe {F : Type*} [NDFunLike F A B] [StarAlgHomClass F R A B] (f : F) :
    ⇑(f : A →⋆ₐ[R] B) = f :=
  rfl
